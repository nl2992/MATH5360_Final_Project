...
def build_market_story_rows(
    ...
    showcase_pr = _extract_pr_marker(
        professor_bundle.get("showcase_pr", professor_bundle["reference_pr"])
    )
    ...
    diag_row = {
        ...
        "Showcase Horizon": str(professor_bundle.get("showcase_scale", reference_vr["time_scale"])),
        ...
        "Showcase PR Rho": showcase_pr["rho"],
...
def build_market_narrative(
    ...
    showcase_tau = int(professor_bundle.get("showcase_tau", professor_bundle["reference_tau"]))
    showcase_scale = str(professor_bundle.get("showcase_scale", professor_bundle["reference_scale"]))
    ...
    if spec.ticker == "TY":
        ...
        lines.append(
            f"The longer diagnostic sweep should also show that the recovery keeps building beyond the reference month-like window, "
            f"with the dense VR curve staying well above the trough by the time we reach {terminal_vr['time_scale']}."
        )
        lines.append(
        ...
        lines.append(
            f"In the extended push-response sweep, the cleaner BTC trend-following picture shows up later around "
            f"{showcase_tau} bars ({showcase_scale}), where rho={showcase_pr['rho']:+.3f}. "
            f"That still supports a shorter and more reactive TF implementation than TY, but it makes the presentation more honest than relying only on the earlier mixed-horizon panel."
        )
