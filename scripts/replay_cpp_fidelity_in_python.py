...
def execute_parametrized_notebook(
    ...
    *,
    ...
    notebook.cells[1].source = (
        ...
        "# Note: place TY-1minHLV.csv under this repo's data/ folder before rerunning.\n"
    )
...
def build_markdown(summary_df: pd.DataFrame, notes: list[str], comparison_df: pd.DataFrame) -> str:
    ...
    lines.extend(
        [
            ...
            "- This standalone repo focuses only on TY 1-minute.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    parser.add_argument("--cpp-results-dir", default="results_cpp_ty_1m", help="Corrected C++ output directory relative to repo root.")
    parser.add_argument("--out-dir", default="results_py_ty_1m", help="Output directory relative to repo root.")
    ...
    jobs = [parse_job(value) for value in args.job] if args.job else [("TY", 1)]
