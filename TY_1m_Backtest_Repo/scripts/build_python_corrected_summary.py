...
OUT_DIR = ROOT / "results_py_ty_1m"

CPP_DIR_MAP: dict[tuple[str, int], Path] = {
    ("TY", 1): ROOT / "results_cpp_ty_1m",
}
...
def build_markdown(summary_df: pd.DataFrame, comparison_df: pd.DataFrame) -> str:
    lines = [
        ...
        "",
        "This report uses the cached Python artifacts rebuilt from the corrected TY 1-minute C++ run.",
        "",
        "## Included runs",
        "- TY 1-minute from `results_cpp_ty_1m`",
        "",
        "## Summary table",
